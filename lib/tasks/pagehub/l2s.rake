namespace :pagehub do
  ...
  namespace :l2s do
    ...
    desc "upgrades page and carbon copy data fields to MEDIUMTEXT"
    task :really_huge_content_fields => :environment do
      require 'dm-migrations/migration_runner'
      migration -1, :really_huge_content_fields do
        up do
          execute "ALTER TABLE pages CHANGE content content MEDIUMTEXT;"
          execute "ALTER TABLE page_carbon_copies CHANGE content content MEDIUMTEXT;"
        end
        down do
        end
      end

      @@migrations.select { |m| m.name == 'really_huge_content_fields' }.first.perform_up
    end
